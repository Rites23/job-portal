      ...
      content={
        <>
          <SidebarGroup>
            ...
            <AsyncIf
              condition={() => hasOrgUserPermission("job_listings:job_listings_create")}
            >
...
async function JobListingMenu({ orgId }: { orgId: string }) {
  ...
  const grouped = jobListings.reduce<Record<string, typeof jobListings>>(
    (acc, j) => {
      const key = String(j.status);
      if (!acc[key]) acc[key] = [];
      acc[key].push(j);
      return acc;
    },
